 ...
 */
export function StickySection({ bgSlot, children, className, bgClassName }: StickySectionProps) {
  return (
    <section className={`relative min-h-[200svh] ${className ?? ""}`}>
      <div className={`sticky top-0 z-0 h-svh w-full overflow-hidden ${bgClassName ?? ""}`}>
        {bgSlot}
      </div>
